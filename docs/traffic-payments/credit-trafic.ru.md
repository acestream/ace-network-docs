...
3. `UserA` пополняет свой счет для оплаты сервисов на 0.2 XAB.
    ...
    Текущий курс XAB/XAC: 1 XAB = 1 XAC.
...
4. `UserA` получает 15 Гб (15360 Мб) трафика от `UserD`. Курс XAB/XAC и стоимость трафика такие же, как в пункте 3. У `UserA` на счету 0.1 XAB - этого достаточно для оплаты 10 Гб трафика. Остальные 5 Гб трафика `UserA` берет у `UserD` в кредит:
