...
**Ace Stream Token (AST)** - утилитарный (utility) токен, предназначенный для управления протоколами и активами Ace Stream DAO.

Ace Stream Token предоставляет своим владельцам следующие привилегированные права:
...
Первичная эмиссия AST осуществляется в объеме 10,000,000,000 AST

Начальный курс токена AST на старте продаж: 1 «Ace Stream Token = 1$

90% токенов от общей эмиссии будут изначально заблокированы на специальном счете ([astLockedPool][3]), доступ к которому есть только у системного смарт-контракта [Unlock System Tokens][4]
...
Средства, полученные от инвесторов, идут на стратегическое развитие Ace Stream DAO, а именно:
...
Приватный токенсейл, в виде прямых продаж токенов Ace Stream Token (AST) стратегическим партнерам и пользователям Ace Stream.
...
Помимо этого, будут доступны OTC сделки: покупка токенов напрямую у организатора (Ace Stream). Продажа токенов организатором будет происходить по рыночной цене, но не ниже номинальной стоимости (1 AST >= 1$).
-->
