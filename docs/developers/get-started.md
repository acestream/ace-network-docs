...
The engine provides HTTP API on the port 6878. The port can be changed by `--http-port` option.

## Connecting to the engine

First, you need to check that the engine is running. The good practice is to do this with `get_version` command:
